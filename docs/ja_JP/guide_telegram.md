...
# 再生時間偽装
Telegramは公式に3秒未満の動画ステッカーを許可しています。
ただし、詳細圧縮オプションの `再生時間偽装`（GUI）または `--duration-spoof`（CLI）を有効にすると、webmビデオステッカーの再生時間を偽装することができ、より長いビデオステッカーをアップロードすることができます。
原理は、Matroska EBML形式の再生時間メタデータを偽装する。

注意：ステッカーの256KBサイズ制限はまだ適用されている。

この方法で作成されたステッカーは、将来再生に失敗するか、削除される可能性があります。
チャンスは低いですが、アカウントが停止されるリスクもあります。