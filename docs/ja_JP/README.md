...
- 現在、Signal、Telegram、WhatsApp、Line（ダウンロードのみ）、Kakao（ダウンロードのみ）、Naver Band（ダウンロードのみ）、OGQ（ダウンロードのみ）、Viber、Discord（ダウンロードのみ）、iMessage（Xcodeスタンプパックプロジェクトを作成）をサポートしています。
...
- MacOS：ダウンロードしたファイルを解凍し、`first_launch.command` を開きます。開けない場合がありますが、「ゴミ箱に入れる」ではなく「完了」をクリックしてください。次に、システム環境設定（または新しいmacOSの場合はシステム設定）を開き、「セキュリティとプライバシー」または「プライバシーとセキュリティ」に移動します。「first_launch.command」がブロックされているというメッセージが表示されますが、「許可」または「開く」をクリックして `first_launch.command` を実行します。その後、`sticker-convert.app` を直接開くことができます。
- Linux：
...
| [WhatsApp](/docs/ja_JP/guide_whatsapp.md)    | ✅ (ログインが必要)                  | ⭕ (ログインが必要、またはSticker Makerでインポート)    |
...
- WhatsApp
    - ダウンロード：sticker-convertでWhatsAppをログインしたら、スタンプをダウンロードできます。又は携帯電話とWhatsApp Webから、手動でスタンプパックを抽出する必要があります。詳細は[/docs/ja_JP/guide_whatsapp.md](/docs/ja_JP/guide_whatsapp.md)をご覧ください。
    - アップロード：sticker-convertでWhatsAppをログインしたら、スタンプをアップロードできます。又は.wastickersファイルを作成し、サードパーティ製アプリ「Sticker Maker」を介してWhatsAppにインポートできます（このリポジトリの作者はSticker Makerとは一切関係ありません）。詳細は"よくある質問"をご覧ください。
- Line
...
輸出オプション:
  ...
  --export-whatsapp     .wastickersファイルに圧縮する、そしてWhatsApp認証した場合はアップロードする
...
圧縮オプション:
  ...
  --duration-max DURATION_MAX
                        最大出力時間（ミリ秒単位）を設定します。
  --duration-spoof      Matroskaビデオ（mkv/webm）の再生時間を限定範囲内で偽装する。3秒を超えるビデオステッカーをTelegramにアップロードするために使用できますが、将来は使えない可能性があります。
...
## 翻訳、Signalデスクトップ版、Viberデスクトップ版、Chromiumなし
docker pull laggykiller/sticker-convert:latest-min-gui
## 翻訳、Signalデスクトップ版、Viberデスクトップ版、Chromiumなし；CLIだけ
...
- [Band] (/docs/ja_JP/guide_band.md)
- [OGQ] (/docs/ja_JP/guide_ogq.md)
- [Viber] (/docs/ja_JP/guide_viber.md)
- [Discord] (/docs/ja_JP/guide_discord.md)
- [Mastodon] (/docs/ja_JP/guide_mastodon.md)
- [Misskey] (/docs/ja_JP/guide_misskey.md)
