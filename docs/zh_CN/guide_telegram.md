...
# 伪造播放时长
Telegram 官方允许使用时长小于 3 秒的影片贴纸。
但是，您可以透过在进阶压缩选项中启用`伪造播放时长`（图形介面）或使用`--duration-spoof`（命令列）参数来伪造 webm 影片贴纸的时长，以上载更长影片贴纸。
其原理是伪造Matroska EBML中的时长元资料。

请注意，贴纸的大小仍须不超过 256KB。

请注意，以这种方式建立的贴纸将来可能无法播放或被删除。
此外，使用这种方法有极小的帐户被封锁的风险。