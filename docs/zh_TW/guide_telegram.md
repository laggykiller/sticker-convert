...
# 偽造播放時長
Telegram 官方允許使用時長小於 3 秒的影片貼紙。
但是，您可以透過在進階壓縮選項中啟用`偽造播放時長`（圖形介面）或使用`--duration-spoof`（命令列）參數來偽造 webm 影片貼紙的時長，以上載更長影片貼紙。
其原理是偽造Matroska EBML中的時長元資料。

請注意，貼紙的大小仍須不超過 256KB。

請注意，以這種方式建立的貼紙將來可能無法播放或被刪除。
此外，使用這種方法有極小的帳戶被封鎖的風險。