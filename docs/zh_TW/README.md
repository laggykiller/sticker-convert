...
- 目前支援 Signal、Telegram、WhatsApp、Line（僅下載）、Kakao（僅下載）、Naver Band（僅下載）、OGQ（僅下載）、Viber、Discord（僅下載）和 iMessage（建立 Xcode 貼紙包專案）。
- 支援靜態和動態貼紙，並支援透明度。
...
| [WhatsApp](/docs/zh_TW/guide_whatsapp.md) | ✅ (需要登入)                    | ✅ (需要登入或以Sticker Maker上傳)             |
...
| [Mastodon](/docs/zh_TW/guide_mastodon.md) | ✅ (需要`token`)                | ✅ (需要`mastodon_cookies`)                   |
| [Misskey](/docs/zh_TW/guide_misskey.md)   | ✅ (需要`token`)                | ✅ (需要`misskey_token`)                      |
...
- WhatsApp
    - 下載：可在sticker-convert登入WhatsApp，以幫助下載貼圖；或手動尋找貼圖包/從手機或 WhatsApp 網頁版提取。請參閱 [/docs/zh_TW/guide_whatsapp.md](/docs/zh_TW/guide_whatsapp.md)。
    - 上載：可在sticker-convert登入WhatsApp，以幫助下載貼圖；或透過第三方應用程式「Sticker Maker」將.wastickers 文件匯入 WhatsApp（本倉庫作者與 Sticker Maker 無任何關聯）。請參閱常見問題。
- Line
    ...
    - 上載：不支援。您需要手動提交貼紙包以獲得批准才能在應用程式中使用。
- Kakao
...
- Discord
    ...
    - 上載: 不支援
- Mastodon
    - 上載: 支援 (例：`https://mastodon.social/`)
    - 上載: 支援。有認證資料時上載至Mastodon, 或壓縮.tar.gz檔案以使用toolctl上載至Mastodon。
- Misskey
    - 上載: 支援 (例：`https://misskey.io/`)
    - 上載: 支援。壓縮.zip檔案並在有認證資料時上載至Misskey。
- iMessage
    - 下載: 不支援
...
輸出選項:
  ...
  --export-whatsapp     創建.wastickers檔案並在有認證資料時上載至WhatsApp
  --export-imessage     創建Xcode專案以供輸入至iMessage
  --export-mastodon     有認證資料時上載至Mastodon, 或壓縮.tar.gz檔案以使用toolctl上載到Mastodon
  --export-misskey      壓縮.zip檔案並在有認證資料時上載至Misskey
...
壓縮選項:
  ...
  --duration-max DURATION_MAX
                        設定最大的動畫長度限制（毫秒）。
  --duration-spoof      允許將 Matroska 影片（mkv/webm）的播放時長偽造在一定範圍內。 
                        可用於上傳時長超過 3 秒的影片貼紙到 Telegram，但未來可能不再支援。
...
認證選項:
  ...
  --whatsapp-setup      設定WhatsApp
  --whatsapp-phone-number WHATSAPP_PHONE_NUMBER
                        不包含括號和加號的電話號碼
                        以驗證碼方式登入時使用
                        例: +1 (234) 567-8901 -> 12345678901
                        (如留空，將以QR code登入)
  ...
  --mastodon-get-auth   從瀏覽器取得Mastodon cookies, 用於上載自訂表情。
  --mastodon-url MASTODON_URL
                        設定Mastodon URL, 用於上載自訂表情。
  --mastodon-cookies MASTODON_COOKIES
                        設定Mastodon cookies, 用於上載自訂表情。
  --misskey-get-auth    從瀏覽器取得Misskey cookies, 用於上載自訂表情。
  --misskey-url MISSKEY_URL
                        設定Misskey URL, 用於上載自訂表情。
  --misskey-token MISSKEY_TOKEN
                        設定Misskey token, 用於上載自訂表情。
  --save-cred           儲存身份驗證數據。
```
</details>
...
## 沒有多語言, Signal 桌面版, Viber 桌面版, Chromium
docker pull laggykiller/sticker-convert:latest-min-gui
## 沒有多語言, Signal 桌面版, Viber 桌面版, Chromium；只有CLI
...
- [Band] (/docs/zh_TW/guide_band.md)
- [OGQ] (/docs/zh_TW/guide_ogq.md)
- [Viber] (/docs/zh_TW/guide_viber.md)
- [Discord] (/docs/zh_TW/guide_discord.md)
- [Mastodon] (/docs/zh_TW/guide_mastodon.md)
- [Misskey] (/docs/zh_TW/guide_misskey.md)
